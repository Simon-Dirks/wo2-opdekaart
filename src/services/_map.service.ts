...
export class _mapService {
  private static _instance: _mapService;
  ...
  constructor() {
    if (_mapService._instance) {
      return _mapService._instance;
    }
    _mapService._instance = this;

    watch(
      () => store.getters["map/getFilteredGeoJson"],
      (filteredGeoJson: AddressesGeoJsonModel) => {
  ...
  public async updateFilter() {
    ...
    for (const feature of geoJson["features"]) {
      ...
      const filteredAddressHasNoDocuments: boolean =
        !filteredAddress?.documents || filteredAddress.documents.length < 0;
  ...
  private _updateSourceData(geoJson: AddressesGeoJsonModel): void {
    if (!this._map) {
      console.warn(
        "Tried to update source data while map was not (yet) initialized"
      );
      return;
    }
  ...
  private _getAddressData(e: any): AddressModel {
    const addressData: AddressModel = JSON.parse(
      JSON.stringify(e.features[0].properties as AddressModel)
    );
