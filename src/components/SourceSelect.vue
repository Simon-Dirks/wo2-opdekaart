...
const onSourceSelect = (sourceId: string, isSelected: boolean) => {
    if(isSelected) {
      ...
      <n-collapse>
        <n-collapse-item title="Bronnen" class="pr-4">
          <div v-for="source in sources">
            <n-checkbox :id="source.id" :checked="selectedSourceIds.has(source.id)" @update:checked="onSourceSelect(source.id, $event)">
              {{ source.label }}
            </n-checkbox>
          </div>
        </n-collapse-item>
