...
<template>
  ...
  <div class="md:grid md:grid-cols-6 h-[50vh] md:h-full">
    ...
    >
      <h1
        class="text-lg mb-4 sticky top-0 px-4 py-4 drop-shadow-2xl bg-slate-700 z-20"
      ...
      <div class="px-4 pt-4">
        ...
        <template v-if="!selectedAddress">
          <address-preview
            ...
            v-for="shownAddress in shownAddresses.slice(
              pageStartElemIdx,
              pageEndElemIdx
            )"
          ></address-preview>
      ...
      <div
        class="w-full sticky bottom-0 px-4 py-2 drop-shadow-2xl bg-slate-700 z-20"
        v-if="!selectedAddress"
      >
